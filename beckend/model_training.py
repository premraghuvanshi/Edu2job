...
def train_career_model(csv_path='data/job_dataset.csv'):
    ...
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Dataset not found at {csv_path}. Please upload it via the Admin Panel.")

    df = pd.read_csv(csv_path)
    

    encoders = {}
    categorical_columns = ['Degree', 'Specialization', 'JobRole']
    ...
    print(f"Success: Model binary and encoders saved at {model_output_path}")
    return accuracy
